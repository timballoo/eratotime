...
- **Any calendar write integration** — no `events.insert`/`update`/`delete` for booking purposes, no attendee management, no `.ics`/iTIP invites sent to invitees, no tokenized self-service reschedule/cancel. All of that is retired in v2; changes and cancellations are handled manually by the organizer (2.4). (The app *does* generate an `.ics` file for the **organizer's own calendar import** in the notification email — that is not an invitation and is not sent to invitees.)
- **Any write integration for Meet link generation is exempt from the above** — the app creates temporary calendar events solely to obtain unique Google Meet links via the Calendar API. These events are ephemeral (optionally deleted immediately after link extraction), never carry attendee information, and are never the booking's calendar event. See section 2.1.
...
- v1 ships with two default meeting types — a 30-minute and a 60-minute slot — but the underlying model is generic (not hardcoded to two types) so more can be added later without a schema change. Each meeting type has:
  ...
  - **Video meetings use Google Meet (confirmed 2026-08).** Two modes, toggleable globally in admin:
    - **Static link** (default): the meeting type carries a fixed Google Meet link in `location_details` (e.g. the organizer's personal Meet link), configured once per meeting type. The app embeds it into the `.ics` calendar-import file's `LOCATION` field, so importing creates the event with the Meet link already attached — no manual typing at event-creation time.
    - **Dynamic links** (`dynamic_meet_links` global toggle): when enabled, a *unique* Google Meet link is generated per booking via the Calendar API using a Service Account. The temporary calendar event is created and the link is extracted (`getHangoutLink()` or `entryPoints[0].uri`); the event is optionally deleted after extraction (`delete_meet_events` toggle). The unique link is stored on `request_log.meet_link` and takes highest priority in the fallback chain (over the static per-type link and the global `meet_link`). This means every booking gets its own Meet room, so meeting overrun cannot cause separate clients to meet each other. On failure, the system falls back to the static per-type link — dynamic generation is never required for a booking to succeed.
    - In both modes, the Gmail address (`meertec.ltd@gmail.com`) never appears in any output sent to invitees. The Service Account authenticates with its own credentials (no domain-wide delegation), and the calendar it writes to is a dedicated "Eratotime Meet Rooms" calendar shared with it.
...
/providers/meet_provider.php          — dynamic Google Meet link generation via Calendar API (Service Account auth)
