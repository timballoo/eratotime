...
- **The organizer's workflow** for every new request: open the notification email (or the admin request log) → click the Google Calendar **quick-add link** (2.5) → create the event in the chosen calendar with the standard event-title convention (2.5) → **add `stephen@meertec.ltd` as an attendee on the event, so the meeting also lands in the primary calendar/inbox** → mark the request `fulfilled` in admin (2.6). Marking `fulfilled` ends the soft-hold's blocking role.
- **Organizer self-attendee step — accepted v1 approach (2026-08):** because the event is created in the calendar-of-record account (default `meertec.ltd@gmail.com`) and the organizer's day-to-day calendar/inbox is `stephen@meertec.ltd`, the organizer adds their own address as an attendee at event-creation time. This is a manual habit, not something the app does. With low expected volume this is acceptable for v1 — no auto-attendance, no dedicated mailbox, no auto-routing.
...
- **Subject/event-title conventions — designed for email-client filtering (2026-08).** The organizer receives requests at `stephen@meertec.ltd` and wants to route them with a mail filter (rule → folder), so every outbound email carries a **stable, filterable subject prefix**, and the quick-add link produces an event with a **stable, searchable title prefix**:
  ...
  - **Google Calendar event title** (the `text=` parameter of the quick-add link): `Eratotime: {meeting type name} — {invitee name}` — so events created through this workflow are distinguishable in the calendar and filterable (search/label) from other meetings.
  - The subject prefixes and the `Eratotime:` title prefix are constants the email composer and quick-add-link builder must not deviate from; volume is low in v1, so a simple email-client filter on sender + prefix into a folder is the accepted mechanism — no app-side mailbox management or auto-routing.
